...
function calcTimes(savedTasks) {
  ...
  const fv = new Set();
  const forward = name => {
    if (fv.has(name)) return;
    fv.add(name);
    const task = savedTasks.find(t => t.name === name);
    if (!task) return;
    const dep = task.resources && savedTasks.some(t => t.name === task.resources) ? task.resources : null;
    if (dep) forward(dep);
    const dur = parseFloat(task.dueDate) || 0;
    T[name].es = dep ? (T[dep]?.ef ?? 0) : 0;
    T[name].ef = T[name].es + dur;
  };
  savedTasks.forEach(t => forward(t.name));
  const PE = Math.max(...Object.values(T).map(t => t.ef), 0);
  const bv = new Set();
  const backward = name => {
    if (bv.has(name)) return;
    bv.add(name);
    const deps = savedTasks.filter(d => d.resources === name);
    deps.forEach(d => backward(d.name));
    const dur = parseFloat(savedTasks.find(t => t.name === name)?.dueDate) || 0;
    T[name].lf = deps.length ? Math.min(...deps.map(d => T[d.name].ls)) : PE;
...
export default function GanttChart({ savedTasks }) {
  if (!savedTasks.length) return (
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', height:'100%', gap:12, color:'#3a3a4a', padding:40 }}>
      <div style={{ fontSize:40 }}>▤</div>
      <div style={{ fontSize:14, color:'#5f5e70' }}>No tasks yet — add some to see the Gantt chart</div>
    </div>
  );
  ...
  return (
    <div style={{ padding:'24px 28px', fontFamily:"'Inter',system-ui,sans-serif" }}>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20 }}>
        <div>
          <div style={{ fontSize:15, fontWeight:700, color:'#f0eff4', letterSpacing:'-0.3px' }}>Gantt chart</div>
          <div style={{ fontSize:12, color:'#5f5e70', marginTop:2 }}>
            Project duration: <span style={{ color:'#00d9a0', fontWeight:600 }}>{days} days</span>
          </div>
        </div>
        <div style={{ display:'flex', gap:8 }}>
          <span style={{ display:'flex', alignItems:'center', gap:5, background:'rgba(255,77,109,0.1)', border:'1px solid rgba(255,77,109,0.2)', borderRadius:6, padding:'4px 10px', fontSize:11, color:'#ff4d6d' }}>● Critical</span>
          <span style={{ display:'flex', alignItems:'center', gap:5, background:'rgba(0,217,160,0.08)', border:'1px solid rgba(0,217,160,0.2)', borderRadius:6, padding:'4px 10px', fontSize:11, color:'#00d9a0' }}>● Normal</span>
        </div>
      </div>

      {/* Grid */}
      <div style={{ background:'#0d0d15', borderRadius:14, border:'1px solid rgba(255,255,255,0.06)', overflow:'hidden' }}>
        {/* Day headers */}
        <div style={{ display:'flex', borderBottom:'1px solid rgba(255,255,255,0.06)', background:'#11111a' }}>
          <div style={{ width:130, flexShrink:0, padding:'10px 16px', fontSize:11, color:'#5f5e70', fontWeight:500, textTransform:'uppercase', letterSpacing:'0.5px' }}>Task</div>
          <div style={{ flex:1, display:'flex' }}>
            {Array.from({ length:days }, (_,i) => (
              <div key={i} style={{ flex:1, minWidth:36, textAlign:'center', padding:'10px 0', fontSize:11, color: i%5===0 ? '#9998a8':'#3a3a4a', borderLeft:'1px solid rgba(255,255,255,0.04)' }}>
                {i+1}
              </div>
            ))}
          </div>
        ...
        {savedTasks.map((t, idx) => {
          const ti     = T[t.name] || {};
          const isCrit = critSet.has(t.name);
          const color  = isCrit ? '#ff4d6d' : COLORS[idx % COLORS.length];
          const leftPct  = ((ti.es||0)/days*100).toFixed(2)+'%';
          const widthPct = ((parseFloat(t.dueDate)||0)/days*100).toFixed(2)+'%';

          return (
            <div key={t.name}
              style={{ display:'flex', borderBottom:'1px solid rgba(255,255,255,0.04)', minHeight:48, alignItems:'center', transition:'background .15s' }}
            ...
            >
              <div style={{ width:130, flexShrink:0, padding:'0 16px', display:'flex', alignItems:'center', gap:8 }}>
                <span style={{ width:8, height:8, borderRadius:'50%', background:color, flexShrink:0, boxShadow: isCrit?`0 0 6px ${color}`:'none' }} />
                <span style={{ fontSize:13, color:'#f0eff4', fontWeight:500 }}>{t.name}</span>
                {isCrit && <span style={{ fontSize:9, background:'rgba(255,77,109,0.15)', color:'#ff4d6d', border:'1px solid rgba(255,77,109,0.25)', borderRadius:4, padding:'1px 5px', fontWeight:600 }}>CRIT</span>}
              </div>
              <div style={{ flex:1, position:'relative', height:48, display:'flex', alignItems:'center' }}>
                {Array.from({ length:days }, (_,i) => (
                  <div key={i} style={{ position:'absolute', left:`${(i/days)*100}%`, top:0, bottom:0, width:1, background:'rgba(255,255,255,0.03)' }} />
                ...
                <div className="gbar"
                  style={{ position:'absolute', left:leftPct, width:widthPct, height:26, borderRadius:7,
                    background: isCrit ? 'linear-gradient(90deg,#ff4d6d,#ff8fa3)' : `linear-gradient(90deg,${color},${color}bb)`,
                    display:'flex', alignItems:'center', paddingLeft:10,
                    animationDelay:`${idx*0.07}s`,
                    boxShadow: isCrit?`0 0 12px rgba(255,77,109,0.35)`:`0 0 8px ${color}44`,
                  }}>
                  <span style={{ fontSize:11, fontWeight:600, color:'#fff', whiteSpace:'nowrap', textShadow:'0 1px 3px rgba(0,0,0,0.5)' }}>
                    {t.name} · {t.dueDate}d
                  </span>
                </div>
              </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12, marginTop:16 }}>
        {[
          { label:'Total tasks',      value:savedTasks.length,   color:'#60a5fa' },
          { label:'Critical tasks',   value:critSet.size,        color:'#ff4d6d' },
          { label:'Project duration', value:`${days}d`,          color:'#00d9a0' },
        ].map(c => (
          <div key={c.label} style={{ background:'#0d0d15', border:'1px solid rgba(255,255,255,0.06)', borderRadius:12, padding:'14px 18px' }}>
            <div style={{ fontSize:11, color:'#5f5e70', marginBottom:6, textTransform:'uppercase', letterSpacing:'0.5px' }}>{c.label}</div>
